...
std::vector<simplexNodePointer> fastPersistence<nodeType>::persistenceByDimension(pipePacket<nodeType> &inData, std::vector<simplexNodePointer> edges, std::vector<simplexNodePointer> pivots, unsigned dimension, comp compStruct, std::string mode, bool recordIntervals)
{
	...
	for (auto columnIndexIter = edges.begin(); columnIndexIter != edges.end(); columnIndexIter++)
	{
		...
		if (it == pivots.end() || (*it)->weight != simplex->weight || (*it)->simplex != simplex->simplex)
		{
			...
			std::vector<simplexNodePointer> faceList = (mode == "homology" ? inData.complex->getAllFacets(simplex) : (inData.complex->simplexType == "alphaComplex" ? inData.complex->getAllCofacets(simplex) : inData.complex->getAllCofacets(simplex->simplex, simplex->weight, pivotPairs, true)));
			...
			while (true)
			{
				...
				else if (pivotPairs.find(pivot) == pivotPairs.end())
				...
				else
				{
					...
					for (simplexNodePointer simp : v[pivotPairs[pivot]])
					{
						...
						std::vector<simplexNodePointer> faces = (mode == "homology" ? inData.complex->getAllFacets(simp) : (inData.complex->simplexType == "alphaComplex" ? inData.complex->getAllCofacets(simp) : inData.complex->getAllCofacets(simp->simplex)));
...
void fastPersistence<nodeType>::runPipe(pipePacket<nodeType> &inData)
{
	...
	for (auto edgeIter = edges[1].begin(); edgeIter != edges[1].end(); edgeIter++)
	{
		...
		if (mappedIndices.size() == 0 || mappedIndices.find(*it) == mappedIndices.end())
			mappedIndices.insert(std::make_pair(*it, mappedIndices.size()));
		int c1 = uf.find(mappedIndices.find(*it)->second);
		it++;
		if (mappedIndices.find(*it) == mappedIndices.end())
			mappedIndices.insert(std::make_pair(*it, mappedIndices.size()));
