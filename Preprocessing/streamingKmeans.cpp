...
void streamingKmeans<nodeType>::runPreprocessor(pipePacket<nodeType> &inData)
{
	...
	*/
	...
	for (int x = (numClusters / 2); x < inData.workData.size() - 1; x++)
	{ // read next point x from the stream
		if (facilities.size() <= maxFacilities)
		{ // first phase goes immediately to reassigning
			...
			{											  // as each point arrives either make it a facility or assign it to one based on delta/f prob
				...
				{ // sorting approx facilities ascending so they can be binary searched
		...
		{				  // we reached max facilities count, now evaluate and raise cost
			f = beta * f; // increasing the cost to add a new centroid
			for (unsigned q = 0; q < facilities.size(); q++)
			{ // summing the points belonging to each cluster
			...
			for (int a = 0; a < approxFacilitiesHat.size(); a++)
			{ // sorting approx facilities ascending so they can be binary searched
			...
			for (int xHat = 0; xHat < summedCentroidVectors.size(); xHat++)
			{ // for each x in K
				...
				{ // add old facility to weighted facility set if weight is high enough
				...
				//			counts.clear();
				for (unsigned k = 0; k < kHatClustered.size(); k++)
				{ // making Khat = to K
					facilities.push_back(kHatClustered[k].first);
				}
				kHatClustered.clear();
				for (unsigned r = 0; r < facilities.size(); r++)
				{ // reassigning approxFacils to Khat
...
template <typename nodeType>
int streamingKmeans<nodeType>::binarySearch(std::vector<std::pair<double, int>> &sorted, std::vector<double> omega, int n, double projection)
{ // dotProd is target
	...
	if (projection <= sorted[0].first)
	{			   // projection = dotProd of current point and omega
...
template <typename nodeType>
double streamingKmeans<nodeType>::randDouble()
{ // random double between 0 and 1  (stack overflow implementation)
	std::mt19937_64 rng;
