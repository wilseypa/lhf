...
template <typename nodeType>
std::vector<std::vector<double>> streamingUtils<nodeType>::kMeans(std::vector<std::vector<double>> &kHat)
{ //"batch"/ normal kmeans clustering to be performed on data
    utils ut;
    int numIterations;
    int numClusters;
    ...
    { // randomly picking centroids from kHat
    ...
    { // iterate until max iteration or no change in classification
...
std::vector<std::vector<double>> streamingUtils<nodeType>::ballKmeans(std::vector<std::vector<double>> &clusters)
{
    ...
    for (int i = 0; i < 2; i++)
    { // randomly picking  2 centroids from clusters
        int index = distribution(gen);
    ...
    for (int i = 0; i < summedCentroidVectors.size(); i++)
    { // moving each centroid to its center of mass per cluster
        for (int d = 0; d < summedCentroidVectors[0].size(); d++)
        {
            summedCentroidVectors[i][d] = summedCentroidVectors[i][d] / counts[i];
        }
