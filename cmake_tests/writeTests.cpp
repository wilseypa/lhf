...
void t_write_functions(std::string &log)
{
	...
	std::string failLog = "";
	std::vector<std::vector<double>> testValueArray{{0.0, 1.0, 2.0}, {2.0, 1.0, 0.0}, {1.0, 1.0, 2.0}, {1.1, 1.1, 1.2}, {0.0, 0.4, 1.0}, {1.5, 1.5, 0.0}};
	std::vector<bettiBoundaryTableEntry> bettiTable;

	// Attempt to write stats
	//  RET: bool
	if (!ws->writeStats("test", "test"))
	{
		failLog += "writeOutput writeStats failed\n";
	}
	...
	//  RET: bool
	if (!ws->writeCSV("test", "test"))
	{
		failLog += "writeOutput writeCSV(str,str) failed\n";
	}

	// Attempt to write CSV
	//  RET: bool
	if (!ws->writeCSV("test", "test", "test"))
	{
		failLog += "writeOutput writeCSV(str,str,str) failed\n";
	}

	// Attempt to write CSV
	//  RET: bool
	if (!ws->writeCSV(testValueArray, "test"))
	{
		failLog += "writeOutput writeCSV(vector<vector<double>>,str) failed\n";
	}

	// Attempt to write CSV
	//  RET: bool
	if (!ws->writeCSV(testValueArray, "test", "test"))
	{
		failLog += "writeOutput writeCSV(vector<vector<double>>,str,str) failed\n";
	}

	// Attempt to write MAT
	//  RET: bool
	if (!ws->writeMAT(testValueArray, "test"))
	{
		failLog += "writeOutput writeMAT failed\n";
	}

	// Attempt to write Barcodes
	//  RET: bool
	if (!ws->writeBarcodes(bettiTable, "barcodeOutput"))
	{
		failLog += "writeOutput writeBarcodes(str,str) failed\n";
	}
...
void t_write_csv_functions(std::string &log)
{
	...
	// Attempt to write stats
	//  RET: bool
	if (!ws->writeCSV(testValueArray, "testCSVOutput"))
	{
		failLog += "writeOutput writeCSV failed\n";
	}
...
void t_write_mat_functions(std::string &log)
{
	...
	// Attempt to write stats
	//  RET: bool
	if (!ws->writeMAT(testValueArray, "testMATOutput"))
	{
		failLog += "writeOutput writeMAT failed\n";
	}
