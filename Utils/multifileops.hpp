...
struct VectorFile
{
    ...
    VectorFile(const std::string &filename) : fileStream(filename, std::ios::in)
    {
...
struct VectorBinaryFile
{
    ...
    size_t dim1;
    size_t dim2;
    VectorBinaryFile(const std::string &filename) : fileStream(filename, std::ios::in | std::ios::binary)
    {
        if (!fileStream.is_open())
            throw std::runtime_error("Failed to open the file.");
...
struct MapBinaryFile
{
    ...
    size_t mapSize;
    size_t vectorSize;
    MapBinaryFile(const std::string &filename) : fileStream(filename, std::ios::in | std::ios::binary)
    {
        if (!fileStream.is_open())
