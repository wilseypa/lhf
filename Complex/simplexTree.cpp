...
simplexTree::simplexTree(double _maxEpsilon, std::vector<std::vector<double>>* _distMatrix, int _maxDim){ // maxEpsilon
	indexCounter = 0;
	std::cout << "simplexTree set indexCounter" << std::endl;
	distMatrix = _distMatrix;
	maxDimension = _maxDim;
	maxEpsilon = _maxEpsilon;           // maxEpsilon
...
void simplexTree::recurseInsert(simplexNode* node, unsigned curIndex, int depth, double maxE, std::set<unsigned> simp){
	...
	simplexNode* temp;
	double curE = 0;   //curE is current Epsilon


	if(runningVectorIndices.size() < runningVectorCount){           //	std::vector<int> runningVectorIndices;
	...
	curE = curE > maxE ? curE : maxE;      //curE is current Epsilon
	...
	if(curE < maxEpsilon){
		...
		simplexNode* insNode = new simplexNode(simp, maxE); // simp is a simplex which is a set of data points' indexs
...
void simplexTree::printTree(simplexNode* headPointer){
	...
	for(int i = 0; i < simplexList.size() ; i++){
		...
		for(auto simplexIter = simplexList[i].begin(); simplexIter != simplexList[i].end(); simplexIter++){
			...
			ut.print1DVector((*simplexIter)->simplex);   // print for std::set<unsigned> simplex;
		}
...
bool simplexTree::insertIterative(std::vector<double> &currentVector, std::vector<std::vector<double>> &window){
	...
	if(streamEval(currentVector, window)) {   // Point is deemed 'significant'  //  execuate sildingwindow::nnBasedEvaluator()
...
void simplexTree::deleteIndexRecurse(int vectorIndex) {
    ...
    // has vectorIndex.
    simplexNode* curNode = root->child;
...
void simplexTree::insert() {
	if(distMatrix->size() == 0){
		...
		//std::cout <<"============csydebuginsert=======================" << std::endl;
		return;
	}
